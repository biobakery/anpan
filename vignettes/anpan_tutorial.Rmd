...
We can use the function `anpan::plot_outcome_tree()` to visually inspect relationship between the tree structure and the outcome, by showing a dot on each leaf shaded according to the outcome:
...
You can sort of see by eye that the phylogeny impacts the outcome. The left side is generally a bit brighter, and close neighbors usually have about the same shade of color.
