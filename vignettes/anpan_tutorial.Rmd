...
We'll cover each of these points in a top-level section. Currently the [Pathway random effects model] section is incomplete. There is also an [Advanced topics] section with some additional information on more complicated details.
...
* zeros are discarded
* The relationship between species abundance and pathway abundance is measured globally by all pathways in the bug
* each pathway has its own intercept, but these intercepts are partially pooled across pathways
  * These terms come from a $Normal(0, \sigma_{int})$ distribution. The prior on $\sigma_{int}$ is a $t_{5, 0, 2.5}$ distribution
* each pathway has its own group effect. These are also pooled across pathways, but they are independent of the pathway intercepts
...
* There are some additional weak priors on several terms. The best place to inspect these in the model code itself in the installation folder `anpan/stan/pwy_ranef.stan`
...
Sometimes you get too many hits from a single bug. This can happen if two conditions are met: 
...
## Configuring `furrr` and `progressr`
...
```{r eval = FALSE}
handlers(handler_beepr(initiate = 1,
                       update = 1,
                       finish = 1,
                       intrusiveness = 1)) # let's do the fork in the garbage disposal!
```
