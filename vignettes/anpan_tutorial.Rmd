...
$$ \textbf{log10(pwy_abd)} \sim \textbf{log10(species_abd) + (1|pwy) + (0 + group|pwy) + } \beta_0$$
...
* the final term $\beta_0$ is the global intercept
...
```{r eval = FALSE}
# Making the gene labels on a plot_results() plot smaller
p[[2]]$theme$axis.text.y = element_text(size = 1)

# Making the points on a plot_outcome_tree() smaller
tree$layers[[2]]$aes_params$size = 3.5
```
