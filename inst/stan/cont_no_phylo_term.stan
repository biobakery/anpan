...
model {
  // likelihood
  target += normal_id_glm_lpdf(Y | Xc, centered_cov_intercept + offset_val, beta, sigma_resid);
...
generated quantities {
  ...
  for (i in 1:N){
    log_lik[i] = normal_id_glm_lpdf(Y[i] | to_matrix(Xc[i]), centered_cov_intercept + offset_val[i], beta, sigma_resid);
  }
}
